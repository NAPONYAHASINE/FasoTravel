...
import { 
  ...
  Mail, Shield, Calendar, Search, Users, Loader2, ChevronDown, Check
} from 'lucide-react';
                ...
                {openDropdown === 'from' ? (
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-3 py-2.5 border-2 border-amber-500 dark:border-amber-500 rounded-xl text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none transition-colors"
                    />
                  ...
                  <button
                    ...
                    className="w-full flex items-center pl-10 pr-3 py-2.5 border-2 border-gray-200 dark:border-gray-600 rounded-xl text-sm bg-white dark:bg-gray-700 hover:border-amber-400 dark:hover:border-amber-600 focus:border-amber-500 dark:focus:border-amber-500 focus:outline-none transition-colors text-left"
                    disabled={stationsLoading}
                  >
                    <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-amber-500" />
                    <span className={from ? 'text-gray-900 dark:text-gray-100 flex-1' : 'text-gray-400 dark:text-gray-500 flex-1'}>
                      {from ? stations.find(s => s.id === from)?.city : (stationsLoading ? 'Chargement...' : 'Ville de départ')}
                    </span>
                ...
                {openDropdown === 'to' ? (
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) => setCitySearch(e.target.value)}
                      placeholder="Rechercher une ville..."
                      className="w-full pl-10 pr-3 py-2.5 border-2 border-green-500 dark:border-green-500 rounded-xl text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none transition-colors"
                    />
                    <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 rotate-180 transition-transform" />
                  </div>
                ) : (
                  <button
                    ...
                    className="w-full flex items-center pl-10 pr-3 py-2.5 border-2 border-gray-200 dark:border-gray-600 rounded-xl text-sm bg-white dark:bg-gray-700 hover:border-green-400 dark:hover:border-green-600 focus:border-green-500 dark:focus:border-green-500 focus:outline-none transition-colors text-left"
                    disabled={stationsLoading}
                  >
                    <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-green-500" />
                    <span className={to ? 'text-gray-900 dark:text-gray-100 flex-1' : 'text-gray-400 dark:text-gray-500 flex-1'}>
                      {to ? stations.find(s => s.id === to)?.city : (stationsLoading ? 'Chargement...' : "Ville d'arrivée")}
                    </span>
                    <ChevronDown className="w-4 h-4 text-gray-400 transition-transform" />
                  </button>
                )}
