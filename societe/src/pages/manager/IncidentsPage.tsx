...
export default function IncidentsPage() {
  const { incidents, trips, updateIncident } = useFilteredData(); // ✅ CORRIGÉ: filtre automatiquement par gareId
  const { user } = useAuth();
  ...
  // ✅ SUPPRIMÉ: Filtre manuel non nécessaire car useFilteredData filtre déjà par gareId
  ...
  const enrichedIncidents = incidents.map(incident => { // ✅ CORRIGÉ: utilise directement incidents (déjà filtrés)
  ...
  const stats = {
    total: incidents.length, // ✅ CORRIGÉ: utilise incidents (déjà filtrés)
    ...
    <div className="p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Consultez et validez les incidents signalés pour votre gare
        </p>
      </div>
