/**
 * HTTP API Client - TransportBF (Unified)
 * 
 * ✅ Centralisé dans /shared - utilisé par Mobile et Societe
 * 
 * Gère:
...
class ApiClient {
  ...
  private logger: Required<ApiClientConfig['logger']>;
  ...
  private async request<T>(
    ...
    try {
      ...
      if (retry < this.maxRetries && this.shouldRetry(lastError)) {
        const delay = 1000 * Math.pow(2, maxAttempts - retry - 2); // Backoff exponentiel
